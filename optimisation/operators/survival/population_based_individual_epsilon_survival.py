...
class PopulationBasedIndividualEpsilonSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, cons_val=None, gen=None, max_gen=None, **kwargs):
        ...
        if problem.n_con > 0 and (type(self.gen) == np.double and type(self.max_gen) == np.double):
            ...
            # Sort solutions by each individual constraint violation
            sorted_cons = np.zeros((len(pop), problem.n_con))
            for i in range(problem.n_con):
                sorted_cons[:, i] = np.sort(cons_pos[:, i])

            # Extract the constraint value of the theta-th solution when ranked by constraint violation from least to
            # most across each individual constraint
            ...
            if self.gen < self.c_gen:
                ...
                self.epsilon = np.zeros(problem.n_con)

            # Re-classify solutions with cons_val below threshold as feasible (for each individual constraint)
            for i in range(problem.n_con):
                cons_pos[cons_pos[:, i] < self.epsilon[i], i] = 0.0

            cons_val_modified = np.sum(cons_pos, axis=1)
