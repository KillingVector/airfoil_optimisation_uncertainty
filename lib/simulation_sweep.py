...
def airfoil_analysis(design, sol_idx, write_quick_history=True, plot=False, print_output=False, debug=False, **kwargs):
    ...
    if design.viable:
            ...
            if current_result:
                if fc_idx < (design.n_fc -1):
                    ...
                    if np.max(current_result.alpha) < alpha_target:
                        ...
                        # Assign results
                        result.c_l[fc_idx] = c_l
                        result.c_d[fc_idx] = c_d
                        result.c_m[fc_idx] = c_m
        ...
        calculate_objectives(design, result, objective, fc_idx)
...
def calculate_objectives(design, result, objective, fc_idx):
    ...
    for idx, obj in enumerate(design.objective):
        if obj == 'max_lift_to_drag':
            temp_result = -result.c_l/result.c_d
            objective[idx] = sum([i for i in temp_result])

            # if objective[idx] < -design.unrealistic_value:
            ...
            temp_result = -weight * result.c_l/result.c_d
            objective[idx] = sum([i for i in temp_result if i != -1])
